...
use crate::{utilidades_index::{leer_index, ObjetoIndex}, tipos_de_dato::{logger::Logger, objetos::tree::Tree}, io::leer_a_string};
...
impl Status {
    pub fn from(logger: Rc<Logger>) -> Result<Status, String> {
        ...
        Ok(Status { logger, index, tree_commit_head, tree_directorio_actual })
    }
    ...
    fn obtener_staging(&self) -> Result<Vec<String>, String> {
        ...
        for objeto_index in &self.index {
            match self.tree_commit_head {
                Some(ref tree) => {
                    let tipo_cambio = if tree.contiene_hijo_por_ubicacion(objeto_index.objeto.obtener_path()) {
                    ...
                    let linea_formateada = format!("{}: {}", tipo_cambio, objeto_index.objeto.obtener_path().display());
                    staging.push(linea_formateada);
                },
                None => {
                    let linea_formateada = format!("nuevo archivo: {}", objeto_index.objeto.obtener_path().display());
                    staging.push(linea_formateada);
                    continue;
                }
            }
