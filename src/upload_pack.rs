...
pub fn upload_pack(
    ...
    if lineas_siguientes[0].clone().contains("done") {
        ...
        let packfile = packfile::Packfile::new()
            .obtener_pack_entero(&(dir.clone().to_string() + "/.gir/objects/")); // obtengo el packfile
    ...
    let respuesta_acks_nak =
        git_io::obtener_ack(have_objs_ids.clone(), dir.clone() + "/.gir/objects/");
    ...
    let packfile = packfile::Packfile::new()
        .obtener_pack_con_archivos(faltantes, &(dir.clone() + "/.gir/objects/"));
    comunicacion.responder_con_bytes(packfile).unwrap();
    Ok(())
}
