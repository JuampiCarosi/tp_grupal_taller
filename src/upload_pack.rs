...
pub fn upload_pack(refs: Vec<String>, dir: String, comunicacion: &mut Comunicacion<TcpStream>) -> Result<(), ErrorDeComunicacion> {
    comunicacion.responder(refs)?; // respondo con las refs 
    // caso push 
    // let refs_a_actualizar = comunicacion.obtener_lineas().unwrap();
    
    let mut wants = comunicacion.obtener_lineas().unwrap(); // obtengo los wants del cliente
    // ------- CLONE --------
