use std::sync::Arc;

use gtk::{glib::Propagation, prelude::*};

use crate::tipos_de_dato::{comandos::check_ignore::CheckIgnore, logger::Logger};

use super::{comando_gui::ComandoGui, dibujar_dialog, info_dialog};

fn ejecutar(input: &gtk::Entry, logger: Arc<Logger>) {
    let input_texto = input.text().to_string();

    let resultado = CheckIgnore::from(vec![input_texto], logger.clone()).ejecutar_gui();

    if let Some(resultado) = resultado {
        info_dialog::mostrar_mensaje("Output:", &resultado);
    }
}

fn dialog(builder: &gtk::Builder, logger: Arc<Logger>) {
    // let tree_view = builder.object::<gtk::TreeView>("hash-object-tree").unwrap();
    let dialog = builder
        .object::<gtk::Dialog>("check-ignore-dialog")
        .unwrap();

    let confirmar = builder
        .object::<gtk::Button>("confirm-check-ignore")
        .unwrap();
    let cancelar = builder
        .object::<gtk::Button>("cancel-check-ignore")
        .unwrap();

    let input = builder.object::<gtk::Entry>("check-ignore-input").unwrap();
    input.set_text("");

    let dialog_clone = dialog.clone();

    let confirmar_id = confirmar.connect_clicked(move |_| {
        ejecutar(&input, logger.clone());
        dialog_clone.hide();
    });

    let dialog_clone = dialog.clone();
    let cancelar_id = cancelar.connect_clicked(move |_| {
        dialog_clone.hide();
    });

    let destroy_id = dialog.connect_destroy_event(|dialog, _| {
        dialog.hide();
        Propagation::Stop
    });

    dibujar_dialog(&dialog);

    confirmar.disconnect(confirmar_id);
    cancelar.disconnect(cancelar_id);
    dialog.disconnect(destroy_id);
}

pub fn render(builder: &gtk::Builder, logger: Arc<Logger>) {
    let event_box = builder.object::<gtk::EventBox>("check-ignore-box").unwrap();

    let builder_clone = builder.clone();
    event_box.connect_button_press_event(move |_, _| {
        let logger_clone = logger.clone();
        dialog(&builder_clone, logger_clone.clone());
        gtk::glib::Propagation::Stop
    });
}
