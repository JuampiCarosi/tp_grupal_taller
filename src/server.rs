...
impl Servidor {
    ...
    fn parse_line(linea: &str, comunicacion: &mut Comunicacion<TcpStream>, dir: &str) -> Result<(), ErrorDeComunicacion> {
        ...
        let refs: Vec<String>;
        comunicacion.enviar_linea(git_io::obtener_linea_con_largo_hex(&(VERSION.to_string() + "\n"))).unwrap();
    ...
    fn obtener_refs_de(dir: PathBuf) -> Vec<String> {
        ...
        if let Ok(mut head) = git_io::obtener_refs_con_largo_hex(dir.join("HEAD"), dir.to_str().unwrap()) {
