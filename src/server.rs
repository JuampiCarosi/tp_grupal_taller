...
impl Servidor { 
    ...
    pub fn new(address: &str) -> Servidor {
        ...
        let (client_stream, socket_addr) = listener.accept().unwrap();
        let dir = match env::current_dir() {
            Ok(current_dir) => current_dir.to_str().unwrap().to_string(),
            Err(_) => '.'.to_string(),
        };
        Servidor { address: address.to_string(), listener, client_stream, socket_addr, dir }
    }

    pub fn server_run(&mut self) -> std::io::Result<()> {
        println!("La socket addr del client: {:?}", self.socket_addr);
        Self::handle_client(&mut self.client_stream)?;
        Ok(())
    }
